...
The expanded writer comparison favored full 2.5 Flash: six accepted passages completed in 2.2–2.7 seconds, including Spanish/Finnish fiction with vocal tags. Partial JSON admission would save little on those responses and add another validation boundary. The complete plan is therefore validated before narration.

Writer routing retains an ordered, configurable list of models. Each model attempt has a 12-second default deadline and model-specific key health. A model with only cooling-down keys yields immediately to an alternative, instead of consuming the audio reserve with a cooldown wait. The route stays on the last successful writer until another failure. Service/access/quota failures and response deadlines allow fallback; cancellation, context exhaustion and invalid requests do not. The full immutable request snapshot, including original response parts and signatures, is counted and submitted to the replacement model. Its metadata supplies a fresh context ceiling; history is never compressed or shortened. Model changes and the actual writer for each plan are archived and reflected in Settings. The Live voice is fixed for the episode.

The current default route is 2.5 Flash (thinking off), 3 Flash preview (minimal), then 2.5 Flash-Lite (off). 3.8 Flash requires low rather than minimal thinking; the helper uses the correct setting when it is configured. 3.1/3.5 Flash-Lite use minimal. Higher-quota models that were unavailable or slow in this account's comparison remain configurable rather than being assumed reliable fallbacks.
...
- [GenerateContent thinking](https://ai.google.dev/gemini-api/docs/generate-content/thinking) and [3.8 Flash](https://ai.google.dev/gemini-api/docs/models/gemini-3.8-flash): model-family-specific lowest thinking levels; 3.8 rejects minimal. [Thought signatures](https://ai.google.dev/gemini-api/docs/generate-content/thought-signatures): preserve full returned response parts in chronological history.
