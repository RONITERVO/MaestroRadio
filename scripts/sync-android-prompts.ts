...
const templates: Record<string, unknown> = { tags: VOICE_TAGS, music: musicInstruction };
for (const styled of [false, true]) for (const expressive of [false, true]) {
  const settings = settingsSchema.parse({ topic: '__TOPIC__', style: styled ? '__STYLE__' : '', expressive, music: false,
