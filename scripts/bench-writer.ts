...
for (const model of process.argv.slice(2).length ? process.argv.slice(2) : ['gemini-3.5-flash-lite', 'gemini-2.5-flash-lite']) {
  ...
  try {
    ...
    const result = { model, keyNumber: keyIndex + 1, thinking: model.startsWith('gemini-2.5') ? 'disabled' : 'minimal', firstTextMs: Math.round(firstText), firstPairMs: Math.round(firstPair), totalMs: Math.round(performance.now() - started), usage, plan: JSON.parse(raw) };
