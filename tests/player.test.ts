...
test('captions use scheduled sample time, with the initial delay and an underrun gap', async () => {
  ...
  player.add(pcm(24000), 48000); assert.equal(audio.starts[2], 5.4);
  audio.currentTime = 5; assert.equal(player.playedSamples, 48000);
  audio.currentTime = 5.9; assert.ok(Math.abs(player.playedSamples - 60000) <= 1);
